...
#
# It ships no configuration, and that is correct rather than an omission -- but
# the two files are not the same kind of thing, and the difference is useful:
#
#   wpa_supplicant_default.conf  regenerated by write_default_conf() before
#                                every use, byte for byte. Shipping a copy
#                                changes nothing.
#   wpa_supplicant_real.conf     read if it is already there. state_try_real
#                                checks conf_exists() first and hands an
#                                existing file straight to wpa_supplicant
